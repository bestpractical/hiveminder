/* THIS IS A PATCHED VERSION OF THE PLUGIN.  PLEASE DO NOT UPGRADE IT WITHOUT
 * ALSO REAPPLYING THE PATCH.
 */
 ...
 *
 * Modified September 4th, 2008 by Shawn M Moore
 * to add a confirmClose callback
 *
...
(function ($) {
	 ...
	 * confirmClose: (Function:null) The callback function used to check whether the dialog should be closed; return true to close it, false to keep it open
	 */
	$.modal.defaults = {
		overlay: 50,
	...
	$.modal.impl = {
		...
		bindEvents: function () {
			...
			$(document).keypress(function (e) {
				// Trap <Escape>
				if ( e.which == 0 && e.originalEvent.keyCode == 27 ) {
					e.preventDefault();
					modal.close();
				}
			});
		 ...
		 *     onClose and confirmClose callbacks will be ignored
		 *
		 * If the confirmClose callback returns false, then this is a no-op
		 */
		close: function (external) {
			...
			if ($.isFunction(this.opts.confirmClose) && !external) {
				if (!this.opts.confirmClose.apply(this, [this.dialog])) {
					return false;
				}
			}
